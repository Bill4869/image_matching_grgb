...
def sift_match(img1, img2, real1, real2):
...
for i in range(len(imgs)):
    for j in range(i, len(imgs)):
        ...
        gray_match, gray_rate = sift_match(gray1, gray2, img1, img2)
        gray_match = cv2.cvtColor(gray_match, cv2.COLOR_BGR2RGB)

        b_match, b_rate = sift_match(b1, b2, img1, img2)
        b_match = cv2.cvtColor(b_match, cv2.COLOR_BGR2RGB)

        g_match, g_rate = sift_match(g1, g2, img1, img2)
        g_match = cv2.cvtColor(g_match, cv2.COLOR_BGR2RGB)

        r_match, r_rate = sift_match(r1, r2, img1, img2)
